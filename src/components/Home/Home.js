...
const Home = ({data}) => {
    ...
    return (
        <div className='shadow-2xl mx-5 quiz-container rounded-md'>
            ...
            <button className='bg-blue-900 text-white mb-10 px-5 py-2 rounded mt-5'> <Link  to={`topic/${id}` }>Start Practice <FaArrowRight className='inline mx-2'></FaArrowRight> </Link>
            </button>
        </div>
    );
};
